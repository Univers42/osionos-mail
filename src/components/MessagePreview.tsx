...
/*   Updated: 2026/06/02 12:42:40 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
export const MessagePreview: React.FC<MessagePreviewProps> = ({
  ...
  const [frameState, setFrameState] = useState({ messageId: '', height: 520 });
  const srcDoc = useMemo(() => message ? messageDocument(message, bodyLoading) : '', [bodyLoading, message]);

  if (message && frameState.messageId !== message.id) {
    setFrameState({ messageId: message.id, height: 520 });
  }
  ...
  const resizeFrame = (event: React.SyntheticEvent<HTMLIFrameElement>) => {
    ...
    setFrameState({ messageId: message.id, height: Math.min(contentHeight + 2, 2400) });
  };
      ...
      <div className="mail-reader-scroll">
        ...
        <section className="mail-reader-frame-wrap" aria-label="Email body">
          <iframe
            ...
            srcDoc={srcDoc}
            style={{ height: frameState.height }}
            onLoad={resizeFrame}
          />
        </section>
